...
static int run_vector(uint32_t vector_id, const snn_golden_case_t *vector) {
    ...
    for (i = 0U; i < 32U; ++i) {
        (void)snn_replay_read(SNN_REPLAY_STATUS);
    }
    ...
    for (i = 0U; i < SNN_REPLAY_TIMEOUT; ++i) {
    ...
    if ((status & SNN_REPLAY_STATUS_DONE) == 0U) {
        xil_printf("VECTOR=%s TIMEOUT status=0x%08x\r\n", vector->name, status);
        return 1;
    }
    ...
    for (i = 0U; i < SNN_GOLDEN_LOGIT_WORDS; ++i) {
        ...
        if (actual != vector->expected_logits_q[i]) {
            xil_printf("VECTOR=%s LOGIT[%d]_FAIL actual=%d expected=%d\r\n",
                       vector->name, i, actual, vector->expected_logits_q[i]);
            failures = 1;
        }
    }
    for (i = 0U; i < SNN_GOLDEN_COUNT_WORDS; ++i) {
        ...
        if (actual != (uint32_t)vector->expected_counts[i]) {
            xil_printf("VECTOR=%s COUNT[%d]_FAIL actual=%d expected=%d\r\n",
                       vector->name, i, actual, vector->expected_counts[i]);
            failures = 1;
        }
    }
    xil_printf("VECTOR=%s %s checksum=0x%08x status=0x%08x\r\n",
               vector->name, failures == 0 ? "PASS" : "FAIL", checksum, status);
    return failures;
}

int main(void) {
    uint32_t version = snn_replay_read(SNN_REPLAY_VERSION);
    uint32_t i;
    int failures = 0;
