...
| ???????? | 50 MHz?? 20 ns??? CNN-LSTM ??? Zynq PS `FCLK_CLK0` ??? 50 MHz???? HLS-3 ? 100 MHz ????? |
...
`cnn_lstm` ?? Vivado ????? `xc7z020clg400-1` ? Zynq PS `FCLK_CLK0=50 MHz`?20 ns?????? SNN Vivado ????????? PS??????? AXI ????????? CNN-LSTM ??? IP ??????

HLS-3 ? 100 MHz ??????????????????????? 50 MHz/20 ns ?????? Vivado synthesis/implementation???? 1688 ???????????50 MHz ?????????? 33.760 ?s???????????
