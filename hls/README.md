...
**HLS-2 status: Q12.6 no-float C++ baseline, deterministic golden-header generator, three-case testbench, and Vitis 2025.1 csim entry are implemented; csim passed.** csynth, Vivado implementation, board replay, and power measurement remain unrun. See [Direct-current Hybrid-SNN and HLS Phase-1 interface contract](../docs/direct_current_hls_architecture.md) for the full scope and terminology.
...
| 模型常量 | HLS-2 为便于 C 仿真，`weight_q[2][32]` 与 `bias_q[2]` 作为 Q12.6 只读参数传入；这不是 AXI/DMA 运行时协议。HLS-3 再决定绑定为版本化 ROM/静态常量。 |
| 输出 | `logits_q[2]`：两个未归一化分类分数，以保留 6 个小数位缩放的有符号宽整数对外表示；读取语义为 raw integer `/ 64`；无 softmax。 |
| HLS-2 已冻结的内部细节 | 乘法/累加器/膜电位使用 HLS-1 全域安全边界确定的显式 `ap_int` 位宽；舍入、溢出边界和 subtract-reset 语义已由三组黄金向量与 Vitis csim 锁定。 |
